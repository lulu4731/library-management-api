...
const getLinkPaymentLibrarian = (amount, name_reader) => {
    var partnerCode = "MOMO"
    var accessKey = "F8BBA842ECF85"
    var secretkey = "K951B6PE1waDMi640xX08PD3vg6EkVlz"
    var requestId = uuid();
...
router.post('/librarian', Auth.authenAdmin, async (req, res, next) => {
    try {
        const { amount, name_reader, books, id_readers } = req.body

        const checkAccount = await LockAccount.checkStatusAccount(id_readers)
        if (checkAccount) {
            if (checkAccount.status === 2) {
                return res.status(400).json({
                    message: `Tài khoản của ${name_reader} đã bị khóa vĩnh viễn`
                })
            } else if (checkAccount.status === 1 && checkAccount.valid === false) {
                return res.status(400).json({
